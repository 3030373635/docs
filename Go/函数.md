...
- 参数：参数由参数变量和参数变量的类型组成，多个参数之间使用`,`分隔。**并且没有默认参数概念，而且是拷贝传参**
...
Go语言中函数内部不能定义有名字的函数，只能定义匿名函数。匿名函数就是没有函数名的函数，匿名函数的定义格式如下：
...
1. 定义在函数内部的函数
2. 该函数的函数体代码包含对外部作用域的引用（非全局作用域）
3. 通常将闭包函数return出来，供外面使用
