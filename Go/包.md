...
**注意：一个文件夹下面直接包含的文件只能归属一个包，同一个包的文件不能在多个文件夹下。包名为`main`的包是应用程序的入口包，且必须有一个main函数。这种包编译后会得到一个可执行文件，而编译不包含`main`包的源代码则不会得到可执行文件。是一个包文件**

# 标识符可见性

同一个包内访问标识符时，无需添加包名前缀，跨包访问标识符时，需要添加包名前缀，例如`fmt.Println("Hello world!")`，就是指调用`fmt`包中的`Println`函数。
...
- alias：引入的包别名，不写别名默认与最后一段路径名相同，例如包名为c
- a/b/c：包所在路径，必须使用双引号包裹起来。
- Go语言中禁止循环导入包。
...
![20170831112523944](assets/images/20170831112523944.png)
...
`godep`是一个通过vender模式实现的Go语言的第三方依赖管理工具，类似的还有由社区维护准官方包管理工具`dep`。**不推荐使用**
...
| go mod download | go.mod文件下载依赖的module到本地cache（默认为$GOPATH/pkg/mod目录） |
|   go mod tidy   | 比对项目文件中引入的依赖与go.mod进行比对，增加缺少的module，删除无用的module |
...
**初始化项目**：在项目目录下执行`go mod init 项目名`
...
`go.mod`文件会记录项目使用的第三方依赖包信息，包括包名和版本，由于`holiday`项目目前还没有使用到第三方依赖包，所以还没有记录任何依赖包信息。

接下来，在项目目录下新建一个`main.go`文件，其内容如下：
...
直接编辑`go.mod`文件，将依赖包和版本信息写入该文件。例如修改`holiday/go.mod`文件内容如下：
...
如果想要导入本地的一个包，并且这个包也没有发布到到其他任何代码仓库，这时候可以在`go.mod`文件中使用`replace`语句将依赖临时替换为本地的代码包。例如在电脑上有另外一个名为`liwenzhou.com/overtime`的项目，它位于`holiday`项目同级目录下：
...
我们可以在`holidy/go.mod`文件中正常引入`liwenzhou.com/overtime`包，然后像下面的示例那样使用`replace`语句将这个依赖替换为使用相对路径（或者绝对路径）表示的本地包。
...
replace liwenzhou.com/overtime  => ../overtime // 也可以是绝对路径
```
