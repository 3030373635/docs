...
**切片的容量是底层数组从切片的第一个元素开始一直到数组最后一个元素的长度**

举个例子，现在有一个数组`a := [8]int{0, 1, 2, 3, 4, 5, 6, 7}`，切片`s1 := a[:5]`，相应示意图如下。![slice_01](https://cos.liuqm.cc/slice_01.png)切片`s2 := a[3:6]`，相应示意图如下：![slice_02](https://cos.liuqm.cc/slice_02.png)
...
s2 := []int{}        //len(s2)=0;cap(s2)=0;s2!=nil 开辟了内存
s3 := make([]int, 0) //len(s3)=0;cap(s3)=0;s3!=nil 开辟了内存
```
